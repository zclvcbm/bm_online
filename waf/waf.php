...
function W_log($log)
{
	$logpath=$_SERVER["DOCUMENT_ROOT"]."/attack_log.txt";
...
function check($str,$v)
{
	...
	if (preg_match("/".$value."/is",$str)==1||preg_match("/".$value."/is",urlencode($str))==1)
		{
			W_log("IP: ".$_SERVER["REMOTE_ADDR"].",时间: ".strftime("%Y-%m-%d %H:%M:%S").",页面:".$_SERVER["PHP_SELF"].",提交方式: ".$_SERVER["REQUEST_METHOD"].",提交数据: ".$str);
