...
if ($v_md5str==$md5string)
{
	if($v_pstatus=="20")
	{
		...
		$db=new MySql();
		$sql = "INSERT INTO tf_paylist (orderid,money,lastdate) " .
				   "VALUES ('$dingdan','$total_fee',now())";
		 $res = $db->query($sql);
		 $sql2="update dindan set payok=1,paymoney=$total_fee,payoktime=now() where order_id='".$dingdan."'";	
		 $res = $db->query($sql2);
		  echo "付款成功";
		//支付成功，可进行逻辑处理！
